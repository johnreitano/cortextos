...
export const dashboardCommand = new Command('dashboard')
  ...
  .action(async (options: { port: string; instance: string; build?: boolean; install?: boolean }) => {
    ...
    // AUTH_URL: without it, NextAuth builds absolute redirect URLs from the
    // Host header it receives — which is localhost:<port> behind the
    // Cloudflare tunnel, so remote logins bounce to a dead localhost URL.
    // Precedence: env > dashboard.env > tunnel.json (written by
    // `cortextos tunnel start` / route setup).
    let authUrl = process.env.AUTH_URL || dashCreds['AUTH_URL'] || '';
    if (!authUrl) {
      try {
        const tunnelCfg = JSON.parse(readFileSync(join(ctxRoot, 'tunnel.json'), 'utf-8'));
        if (typeof tunnelCfg.tunnelUrl === 'string' && !tunnelCfg.tunnelUrl.includes('.cfargotunnel.com')) {
          authUrl = tunnelCfg.tunnelUrl;
        }
      } catch { /* no tunnel configured — local-only dashboard needs no AUTH_URL */ }
    }
