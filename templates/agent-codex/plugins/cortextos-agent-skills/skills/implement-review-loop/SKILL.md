...
```
This reads the record it needs, then deletes the state file — closing the trust window immediately, before validating the record, computing diffs, or touching the worktree — re-validates the record, computes the diff stat and commit count, attempts to remove the worktree (a failure here is reported but does not block the approval request — you may need to clean it up manually), and automatically files a `create-approval` request (category `deployment`) summarizing the change. Follow the rest of the `approvals` skill's workflow from here — block your task on the approval ID, notify the user, wait for the inbox decision.
